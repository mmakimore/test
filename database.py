...
def update_user(user_id, **kw):
    ok = ['full_name','username','phone','card_number','bank','role','is_active','balance',
...
def search_users(query: str, limit: int = 30, offset: int = 0):
    """
    Поиск пользователей по имени, телефону, username или telegram_id.
    query может содержать '@' — он будет проигнорирован.
    """
    q = (query or "").strip()
    if q.startswith("@"):
        q = q[1:]
    ...
    with get_connection() as conn:
        c = conn.cursor()
        # если запрос — число, пробуем искать telegram_id или id точно
        if q.isdigit():
            return [dict(r) for r in c.execute(
                "SELECT * FROM users WHERE telegram_id=? OR id=? OR phone LIKE ? OR username LIKE ? OR full_name LIKE ? "
                "ORDER BY created_at DESC LIMIT ? OFFSET ?",
                (int(q), int(q), like, like, like, limit, offset)
            ).fetchall()]
        return [dict(r) for r in c.execute(
            "SELECT * FROM users WHERE phone LIKE ? OR username LIKE ? OR full_name LIKE ? "
            "ORDER BY created_at DESC LIMIT ? OFFSET ?",
            (like, like, like, limit, offset)
        ).fetchall()]

def count_search_users(query: str) -> int:
    q = (query or "").strip()
    if q.startswith("@"):
        q = q[1:]
    like = f"%{q}%"
    with get_connection() as conn:
        ...
        if q.isdigit():
            return int(c.execute(
                "SELECT COUNT(*) FROM users WHERE telegram_id=? OR id=? OR phone LIKE ? OR username LIKE ? OR full_name LIKE ?",
                (int(q), int(q), like, like, like)
            ).fetchone()[0])
        return int(c.execute(
            "SELECT COUNT(*) FROM users WHERE phone LIKE ? OR username LIKE ? OR full_name LIKE ?",
            (like, like, like)
        ).fetchone()[0])
